...
import { FaArrowRight, FaEnvelope, FaEye, FaEyeSlash, FaGoogle, FaLock, FaUserPlus } from "react-icons/fa";
		...
		<section className="bg-slate-50 px-6 py-12 text-slate-950 lg:px-10">
			<div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
				<div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
					<p className="text-sm font-bold uppercase text-green-600">Sign up</p>
					<h1 className="mt-2 text-3xl font-bold">Create your Patzen account</h1>
					...
					>
						<label className="grid gap-2 font-bold text-slate-800">
							First name
							<input className="h-12 rounded-md border border-slate-200 px-4 outline-none focus:border-blue-500" name="firstName" autoComplete="given-name" required />
						</label>
						<label className="grid gap-2 font-bold text-slate-800">
							Last name
							<input className="h-12 rounded-md border border-slate-200 px-4 outline-none focus:border-blue-500" name="lastName" autoComplete="family-name" required />
						</label>
						<label className="grid gap-2 font-bold text-slate-800 sm:col-span-2">
							Email
						...
						<label className="grid gap-2 font-bold text-slate-800 sm:col-span-2">
							...
							<span className="flex h-12 items-center gap-3 rounded-md border border-slate-200 px-4 focus-within:border-blue-500">
								...
								<input className="min-w-0 flex-1 outline-none" name="password" type={isPasswordVisible ? "text" : "password"} autoComplete="new-password" minLength={8} required />
								<button
									type="button"
				...
				<div className="flex min-h-[520px] flex-col justify-between rounded-lg bg-blue-950 p-8 text-white">
					<div>
						...
						<p className="mt-8 text-sm font-bold uppercase text-green-400">Trade ready</p>
						<h2 className="mt-3 font-['Inter'] text-4xl font-bold leading-tight sm:text-5xl">Build your account before the next job starts</h2>
						<p className="mt-5 max-w-md text-lg leading-8 text-blue-100">Save carts, see order history, and keep delivery details ready for fast checkout.</p>
					</div>
					<a href="/login" className="mt-8 inline-flex w-fit items-center gap-2 font-bold text-green-300 hover:text-green-200">
						Already have an account? Sign in <FaArrowRight />
					</a>
				</div>
			</div>
		</section>
